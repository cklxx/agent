...
### 3. 代码Agent工作流 (`src/code_agent_workflow.py`)
...
class CodeAgentWorkflow:
    ...
    async def execute_task(self, task_description: str, max_iterations: int = 5) -> Dict[str, Any]:
        """
        执行代码任务，支持三阶段模式：
        1. 前置信息收集阶段 (pre_analysis)
        2. 任务实施阶段 (implementation) 
        3. 验证确认阶段 (verification)
        """
```
...
### RAG 集成
通过 RAGFlow 集成，支持在输入中提及文件进行文档检索。

### 人机协作
支持交互式修改研究计划，可使用自然语言调整AI的执行策略。
...
### Q: 如何配置不同的LLM提供商？
A: 在 `conf.yaml` 中配置不同的模型，系统通过 LiteLLM 支持大多数主流提供商。

### Q: 如何添加新的搜索引擎？
A: 在 `.env` 中设置 `SEARCH_API` 变量，支持 tavily、duckduckgo、brave_search、arxiv。

### Q: 如何自定义代码执行流程？
A: 修改 `src/code_agent_workflow.py` 中的 CodeAgentWorkflow 类，支持三阶段执行模式。

### Q: Web UI 如何与后端通信？
A: 前端通过 SSE (Server-Sent Events) 与 FastAPI 后端实时通信。
