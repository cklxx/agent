...
class SandboxManager:
    ...
    async def execute_python_code(
        self, code: str, input_data: Any = None, timeout: Optional[int] = None
    ) -> Dict[str, Any]:
        """执行Python代码"""
...
"""
        ...
        for i, test_case in enumerate(test_cases):
            try:
                ...
                results["details"].append({"test_case": i, "result": result})

            except Exception as e:
                results["failed"] += 1
                results["details"].append(
                    {"test_case": i, "result": {"success": False, "error": str(e)}}
                )

        return results

    async def _run_single_test(
        self, test_code: str, test_case: Dict[str, Any]
    ) -> Dict[str, Any]:
        """运行单个测试用例"""
        import json
