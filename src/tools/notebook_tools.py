...
def _load_notebook(notebook_path: str) -> Dict[str, Any]:
    ...
    if not os.path.isabs(notebook_path):
        raise ValueError(
            f"notebook_path must be an absolute path, got: {notebook_path}"
        )
    ...
    if not notebook_path.endswith(".ipynb"):
        raise ValueError(
            f"File must be a Jupyter notebook (.ipynb), got: {notebook_path}"
        )
    ...
    try:
        ...
        if edit_mode == "delete":
            ...
            if cell_type not in ["code", "markdown"]:
                return (
                    f"Error: cell_type must be 'code' or 'markdown', got: {cell_type}"
                )
            ...
            if target_type not in ["code", "markdown"]:
                return (
                    f"Error: cell_type must be 'code' or 'markdown', got: {target_type}"
                )
