...
def print_phase_header(phase_name: str, description: str):
    """打印阶段标题"""
    phase_icons = {"pre_analysis": "🔍", "implementation": "⚙️", "verification": "🔬"}
...
async def demonstrate_task_planning():
    ...
    for i, task in enumerate(test_tasks, 1):
        ...
        for phase_name, phase_steps in phases.items():
            print_phase_header(
                phase_name, f"{phase_name.upper()} - {len(phase_steps)} 步骤"
            )
            ...
            for j, step in enumerate(phase_steps, 1):
                ...
                if verification:
                    print(
                        f"     ✅ 验证: {', '.join(verification[:2])}{'...' if len(verification) > 2 else ''}"
                    )
                print()
