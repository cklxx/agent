...
""")
    ...
    def test_rag_search_success_scenario(self):
        ...
        # 测试搜索功能
        with patch('src.tools.rag_enhanced_search_tools.RAGEnhancedSearchTools._get_rag_results', mock_rag_search):
    ...
    def test_rag_workspace_filtering_scenario(self):
        ...
        # 模拟过滤函数
        def filter_results_by_workspace(results: List[Dict], workspace_path: Path) -> List[Dict]:
            def is_path_in_workspace(file_path: str) -> bool:
                try:
                    resolved_path = Path(file_path).resolve()
                    workspace_resolved = workspace_path.resolve()
                    return (
                        resolved_path == workspace_resolved or 
                        workspace_resolved in resolved_path.parents
                    )
                except Exception:
                    return False
        ...
        # 验证过滤结果
        assert len(filtered_results) == 2, f"应该保留2个内部文件，实际: {len(filtered_results)}"
        
        # 验证保留的都是内部文件
        for result in filtered_results:
            file_path = Path(result["file_path"]).resolve()
            workspace_resolved = self.workspace.resolve()
            assert workspace_resolved in file_path.parents or file_path == workspace_resolved, \
        ...
        print(f"   过滤率: {((len(mixed_results)-len(filtered_results))/len(mixed_results))*100:.1f}%")
    ...
    def test_rag_async_compatibility_scenario(self):
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    with ThreadPoolExecutor() as executor:
                        future = executor.submit(asyncio.run, mock_async_rag_search(query, workspace))
        ...
        assert "test async query" in result[0]["content"] or "sync fallback" in result[0]["content"], \
            "结果应该包含查询内容"
    ...
    def test_rag_result_ranking_scenario(self):
        ...
        # 验证具体顺序
        assert ranked_results[0]["title"] == "main.py", "第一位应该是main.py（相似度最高）"
    ...
    def test_rag_context_integration_scenario(self):
        ...
        class MockRAGContextManager:
            ...
            def add_function_search_context(self, function_name: str, results: List[Dict]) -> str:
        ...
        # 添加搜索上下文
        search_context_id = context_manager.add_rag_search_context(search_results, "main function")
        ...
        func_context_id = context_manager.add_function_search_context("load_config", function_results)
