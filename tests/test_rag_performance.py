...
"""
    ...
    def test_path_validation_performance(self):
        ...
        for level1 in range(3):
            for level2 in range(5):
                for file_idx in range(10):
                    path = str(self.workspace / f"level1_{level1}" / f"level2_{level2}" / f"module_{file_idx}.py")
        ...
        assert paths_per_second > 100, f"处理速度应该 > 100 路径/秒，实际: {paths_per_second:.0f}"

    def test_result_filtering_performance(self):
        """测试结果过滤性能"""
        print("🧪 测试结果过滤性能")
        
        def filter_rag_results_by_workspace(results: list, workspace_path: Path) -> list:
        ...
        for level1 in range(3):
            for level2 in range(5):
                for file_idx in range(10):
                    file_path = str(self.workspace / f"level1_{level1}" / f"level2_{level2}" / f"module_{file_idx}.py")
        ...
        filtered_results = filter_rag_results_by_workspace(large_results, workspace_resolved)
        ...
        elapsed_time = end_time - start_time
        memory_diff = end_memory - start_memory
        results_per_second = len(large_results) / elapsed_time if elapsed_time > 0 else 0
        
        print(f"✅ 结果过滤性能测试完成:")
        print(f"   原始结果数: {len(large_results)}")
        print(f"   过滤后结果数: {len(filtered_results)}")
        print(f"   过滤率: {((len(large_results)-len(filtered_results))/len(large_results))*100:.1f}%")
        print(f"   总耗时: {elapsed_time:.3f}秒")
        ...
        assert len(filtered_results) == expected_internal, f"应该保留{expected_internal}个内部结果"
        assert results_per_second > 50, f"处理速度应该 > 50 结果/秒，实际: {results_per_second:.0f}"
    ...
    def test_concurrent_search_performance(self):
        ...
        # 性能断言
        assert len(serial_results) == len(parallel_results) == len(queries), "结果数量应该相同"
