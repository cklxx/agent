...
""")
    ...
    def test_workspace_tools_creation(self):
        ...
        try:
            ...
            expected_tools = ['view_file', 'list_files', 'glob_search', 'grep_search', 'semantic_search']
    ...
    def test_glob_search_rag_integration(self, mock_rag_glob):
        ...
        try:
            # 设置mock返回值
            mock_rag_glob.func = AsyncMock(return_value="""
## 🔍 传统文件系统搜索结果
搜索范围: {workspace}
找到文件:
- src/database.py
...
""".format(workspace=str(self.workspace)))
    ...
    def test_grep_search_rag_integration(self, mock_rag_grep):
        ...
        try:
            # 设置mock返回值
            mock_rag_grep.func = AsyncMock(return_value="""
## 🔍 传统文件系统搜索结果
搜索范围: {workspace}
匹配内容:
...
""".format(workspace=str(self.workspace)))
    ...
    def test_semantic_search_integration(self, mock_semantic):
        ...
        try:
            # 设置mock返回值
            mock_semantic.func = AsyncMock(return_value="""
...
""".format(workspace=str(self.workspace)))
    ...
    def test_workspace_file_operations(self):
        ...
        try:
            # 验证测试文件是否正确创建
            assert (self.workspace / "src" / "database.py").exists(), "数据库模块文件应该存在"
            ...
            assert "DatabaseManager" in db_content, "数据库文件应该包含DatabaseManager类"
            ...
            assert "authenticate_user" in auth_content, "认证文件应该包含authenticate_user方法"
