...
""")
    ...
    def test_workspace_path_validation(self):
        ...
        try:
            ...
            assert not is_external, f"外部路径应该被识别为在workspace外: {external_path}"
    ...
    def test_workspace_path_resolution(self):
        ...
        try:
            ...
            assert resolved_external == str(self.workspace), "workspace外路径应该被重定向"
            print(f"✅ workspace外路径重定向正确: {abs_external} -> {resolved_external}")
    ...
    def test_rag_results_filtering(self):
        ...
        try:
            ...
            # 验证过滤结果
            assert len(filtered) == 2, f"应该过滤掉1个外部文件，实际过滤后数量: {len(filtered)}"
            
            # 验证保留的文件都在workspace内
            for result in filtered:
                file_path = result['file_path']
                # 结果应该是相对路径
                assert not os.path.isabs(file_path), f"过滤后的路径应该是相对路径: {file_path}"
