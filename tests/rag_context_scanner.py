...
class RAGContextScanner:
    ...
    def scan_rag_files(self) -> Dict[str, Any]:
        ...
        rag_keywords = [
            ...
            'ContextType', 'RAG_CODE', 'RAG_SEMANTIC', 'rag_context'
        ]
            ...
            for file in files:
                if file.endswith(('.py', '.md', '.txt', '.json', '.yaml', '.yml')):
                    ...
                    try:
                        ...
                        # 检查是否包含RAG相关关键词
                        rag_keyword_matches = sum(1 for keyword in rag_keywords if keyword in content)
                        ...
                        if rag_keyword_matches > 0:
                            ...
                            summary['total_rag_functions'] += len(analysis.get('rag_functions', []))
                            summary['total_rag_classes'] += len(analysis.get('rag_classes', []))
                            summary['total_context_types'] += len(analysis.get('rag_context_types', []))
                            summary['total_test_methods'] += len(analysis.get('test_methods', []))
                            summary['file_types'][analysis.get('file_type', 'unknown')] += 1
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if 'rag' in alias.name.lower() or 'context' in alias.name.lower():
                    ...
                    if node.module and ('rag' in node.module.lower() or 'context' in node.module.lower()):
                        for alias in node.names:
                            analysis['imports'].append({
                                'type': 'from_import',
                                'module': node.module,
                ...
                # 分析函数定义
                    ...
                    if any(keyword in node.name.lower() for keyword in ['rag', 'context', 'search', 'retriev']):
                ...
                # 分析类定义
                    ...
                    if any(keyword in node.name.lower() for keyword in ['rag', 'context', 'retriev', 'search']):
                    ...
                    for target in node.targets:
                        if isinstance(target, ast.Name) and any(
                            ...
                            analysis['context_instances'].append({
                                'name': target.id,
                                'line': node.lineno,
                                'type': self._get_node_name(node.value) if hasattr(node.value, 'id') else 'unknown'
                            })
        ...
        for i, line in enumerate(lines, 1):
            ...
            if 'contexttype' in line_lower and 'rag' in line_lower:
                analysis['rag_context_types'].append({
                    'line': i,
                    'content': line_stripped[:100]  # 限制长度
                })
            ...
            for keyword in rag_keywords:
                if keyword in line:
                    analysis['rag_keywords'].append({
                        'keyword': keyword,
                        'line': i,
                        'context': line_stripped[:50] + '...' if len(line_stripped) > 50 else line_stripped
                    })
    ...
    def generate_context_report(self) -> str:
        ...
        report.append(f"- RAG相关函数总数: **{results['summary']['total_rag_functions']}**")
        report.append(f"- RAG相关类总数: **{results['summary']['total_rag_classes']}**")
        report.append(f"- Context类型定义: **{results['summary']['total_context_types']}**")
        ...
        for file_path in results['rag_files']:
            ...
            if rag_functions > 0 or rag_classes > 0 or test_methods > 0:
                report.append(f"- RAG功能: {rag_functions} 个函数, {rag_classes} 个类, {test_methods} 个测试")
        ...
        for file_path, analysis in results['file_analysis'].items():
            ...
            if analysis.get('rag_functions'):
                ...
                for func in analysis['rag_functions'][:10]:  # 最多显示10个
                    ...
                    report.append(f"- `{async_mark}{func['name']}()` (第{func['line']}行){test_mark}")
            ...
            if analysis.get('rag_classes'):
                ...
                for cls in analysis['rag_classes']:
                    ...
                    report.append(f"- `{cls['name']}` (第{cls['line']}行, {method_count} 个方法)")
        ...
        has_managers = sum(1 for analysis in results['file_analysis'].values() 
        ...
        report.append(f"- RAG文件覆盖率: {(total_files / max(results['files_scanned'], 1) * 100):.1f}%")
